...
public:
	...
	static scalar norm(Point<scalar>&& first, const Point<scalar>& second)
	{
		return sqrt(pow(second.x() - first.x(), 2) + pow(second.y() - first.y(), 2) + pow(second.z() - first.z(), 2));
	}

	static scalar norm(const Point<scalar>& first, Point<scalar>&& second)
	{
		return sqrt(pow(second.x() - first.x(), 2) + pow(second.y() - first.y(), 2) + pow(second.z() - first.z(), 2));
	}

	static scalar norm(Point<scalar>&& first, Point<scalar>&& second)
	{
		return sqrt(pow(second.x() - first.x(), 2) + pow(second.y() - first.y(), 2) + pow(second.z() - first.z(), 2));
	}
